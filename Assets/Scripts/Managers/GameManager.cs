...
public class GameManager : MonoBehaviour {
	...
	void Start () 
	{
        ...
        var loginScript = FindObjectOfType<Login>();
        loginScript.Gamer.Scores.Domain("private").ListUserBestScores()
        .Done(listUserBestScoresRes => {
            if (listUserBestScoresRes.Count == 0)
            {
                hightScore = 0;
            }
            else
            {
                hightScore = (int)listUserBestScoresRes["normalMode"].Value;
            }
        }, ex => {
